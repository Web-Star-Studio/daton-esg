...
class CaptacaoRow(Base):
    ...
    with the corresponding GRI code FK.

    `sessao` uses the existing document_directories.DOCUMENT_DIRECTORIES keys.
    `gri_code` is a nullable FK to gri_standards.code; some rows have no GRI
    code assigned.
    """
    ...
    gri_code: Mapped[str | None] = mapped_column(
        String(20),
        ForeignKey("gri_standards.code", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    descricao: Mapped[str] = mapped_column(Text, nullable=False, default="")
    fonte_documental: Mapped[str] = mapped_column(
